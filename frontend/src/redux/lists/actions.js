...
export const fetchListCategories = () => {
  return async (dispatch) => {
    ...
    try {
      let res = await debouncedFetchListsCategories('/categories');
      ...
      res.data.data.forEach((res) => {
        _temp.push({
          value: res.id,
          label: res.name,
          target: { value: res.id, name: 'category' },
        });
      });
...
export const fetchListTalents = () => {
  return async (dispatch) => {
    ...
    try {
      let res = await debouncedFetchListsTalents('/talents');
      ...
      res.data.data.forEach((res) => {
        _temp.push({
          value: res.id,
          label: res.name,
          target: { value: res.id, name: 'talent' },
        });
      });
...
export const fetchListEvents = () => {
  return async (dispatch) => {
    ...
    try {
      let res = await debouncedFetchListsEvents('/events');
      ...
      res.data.data.forEach((res) => {
        _temp.push({
          value: res.id,
          label: res.title,
          target: { value: res.id, name: 'event' },
        });
      });
